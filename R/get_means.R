...
get_means <- function(data, scale_list, reverse_score = TRUE, rename_reversed_items = FALSE, order = 1){
...
get_means <- function(data, scale_list, reverse_score = TRUE, rename_reversed_items = FALSE, order = 1){
  ...
  for(scale_name in scale_list){
    if(order == 1){
      ...
      # For second order scales, get the names of first order scales
    ...
    if(length(item_cols) > 0){
      if(reverse_score && order == 1) { # Only apply reverse scoring for first order scales
